...
fn main() {
    ...
    println!("Fetch certificate..");
    match acme::lets_encrypt(&email, &domain) {
        Err(err) => {
            println!("Acme error {}, aborting", err); 
            process::exit(1);
        },
        _ => {}
    }
    ...
    /* Spawn port 80 service */
    std::thread::spawn(move || {
        println!("Spawning port 80 service");
        let index = warp::fs::dir(www_root);
